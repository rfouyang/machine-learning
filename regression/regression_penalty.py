...
def regression_l1_sklearn():
    xtrain, xtest, ytrain, ytest = get_data(show=False)

    model = sklearn.linear_model.Lasso(alpha=0.5)
    model.fit(xtrain, ytrain)
...
def regression_l2_tensorflow():
    ...
    l2_regularizer = tf.contrib.layers.l2_regularizer(scale=0.05)
    weights = tf.trainable_variables()
    penalty = tf.contrib.layers.apply_regularization(l2_regularizer, weights)

    loss = tf.reduce_mean(tf.square(target - pred)) + penalty
...
def regression_l1_tensorflow():
    ...
    weights = tf.trainable_variables()
    penalty = tf.contrib.layers.apply_regularization(l1_regularizer, weights)
...
def main():
    ...
    regression_l1_sklearn()

    #regression_l2_tensorflow()
    # regression_l1_tensorflow()
